...
import { type Profile, updateProfileAction, deleteProfileAction } from "./actions"
...
export function UserTable({
  ...
  const handlePageChange = (page: number) => {
    updateUrl({ page, search: searchQuery, role: roleFilter, status: statusFilter })
  ...
  const openConfirmDialog = (
    ...
    setConfirmDialog({ open: true, type, id, currentValue, newValue, userName })
  }

  const handleConfirm = async () => {
    if (confirmDialog.type === "role") {
      const { error } = await updateProfileAction(confirmDialog.id, { role: confirmDialog.newValue })
      if (error) {
      ...
      const { error } = await updateProfileAction(confirmDialog.id, { is_blocked: confirmDialog.newValue === "Blocked" })
      if (error) {
  ...
  const columns = [
    ...
    { key: "username", label: "Username" },
    {
      ...
      render: (value: unknown, row: Record<string, unknown>) => {
        ...
        const userName = (row.fullname as string) || (row.username as string) || "Unknown user"
        return (
          <DropdownMenu>
            <DropdownMenuTrigger className="focus:outline-none">
              <div className="cursor-pointer hover:opacity-80 flex items-center">
                <Badge variant="outline" className={roleColors[role.toLowerCase()] ?? "bg-secondary text-secondary-foreground border-border"}>
                  {formatRole(role)}
                </Badge>
            ...
            <DropdownMenuContent align="start">
              {["user", "admin"].filter((r) => r !== role.toLowerCase()).map((r) => (
                <DropdownMenuItem key={r} onClick={() => openConfirmDialog("role", id, role, r, userName)} className="cursor-pointer">
                  {formatRole(r)}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
      ...
      render: (value: unknown, row: Record<string, unknown>) => {
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start">
              {status !== "Active" && (
                <DropdownMenuItem onClick={() => openConfirmDialog("status", id, status, "Active", userName)} className="cursor-pointer">Active</DropdownMenuItem>
              )}
              {status !== "Blocked" && (
                <DropdownMenuItem onClick={() => openConfirmDialog("status", id, status, "Blocked", userName)} className="cursor-pointer">Blocked</DropdownMenuItem>
              )}
            </DropdownMenuContent>
          </DropdownMenu>
        )
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => openEditDialog(row)} className="cursor-pointer">
                <UserPen className="h-4 w-4 mr-2" />
                Edit User
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={() => openDeleteDialog(id, userName)} className="cursor-pointer text-destructive focus:text-destructive">
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={confirmDialog.open} onOpenChange={(open) => setConfirmDialog((prev) => ({ ...prev, open }))}>
        <DialogContent showCloseButton={false}>
          <DialogHeader>
            ...
            <DialogDescription>
              Apakah Anda yakin ingin mengubah {confirmDialog.type === "role" ? "role" : "status"}{" "}
              <strong>{confirmDialog.userName}</strong> dari <strong>{confirmDialog.currentValue}</strong> menjadi{" "}
      ...
      {/* Edit Dialog */}
      <Dialog open={editDialog.open} onOpenChange={(open) => setEditDialog((prev) => ({ ...prev, open, showConfirm: false }))}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editDialog.showConfirm ? "Konfirmasi Perubahan" : "Edit Pengguna"}</DialogTitle>
            {editDialog.showConfirm && (
              <DialogDescription>Apakah Anda yakin ingin menyimpan perubahan ini?</DialogDescription>
            )}
          </DialogHeader>
          {!editDialog.showConfirm ? (
            <div className="grid gap-4 py-4">
              <div className="grid gap-2">
                ...
                <Input id="fullname" value={editDialog.fullname} onChange={(e) => setEditDialog((prev) => ({ ...prev, fullname: e.target.value }))} />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="username">Username</Label>
                <Input id="username" value={editDialog.username} onChange={(e) => setEditDialog((prev) => ({ ...prev, username: e.target.value }))} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  <Label htmlFor="role">Role</Label>
                  <Select value={editDialog.role} onValueChange={(value) => setEditDialog((prev) => ({ ...prev, role: value }))}>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditDialog((prev) => ({ ...prev, open: false, showConfirm: false }))}>Batal</Button>
            <Button onClick={handleEditSave}>{editDialog.showConfirm ? "Ya, Simpan" : "Simpan"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={deleteDialog.open} onOpenChange={(open) => setDeleteDialog((prev) => ({ ...prev, open, confirmText: "" }))}>
        <DialogContent showCloseButton={false}>
          <DialogHeader>
            ...
            <DialogDescription>
              Anda akan menghapus pengguna <strong>{deleteDialog.userName}</strong>. Tindakan ini tidak dapat dibatalkan.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleDeleteUser} disabled={deleteDialog.confirmText !== "Delete"}>Hapus</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
